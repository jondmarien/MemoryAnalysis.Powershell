BeforeAll {
    $script:ModuleRoot = Join-Path $PSScriptRoot "..\..\PowerShell.MemoryAnalysis\publish"
    $script:ModulePath = Join-Path $script:ModuleRoot "MemoryAnalysis.psd1"
    $script:HasMamlHelp = Test-Path (Join-Path $script:ModuleRoot "en-US")
...
Describe "MemoryAnalysis Module" {
    ...
    Context "Cmdlet Availability" {
        ...
        It "Should export Analyze-ProcessTree alias for Test-ProcessTree" {
    ...
    Context "Cmdlet Help" -Skip:(-not $script:HasMamlHelp) {
    ...
    Context "Error Handling" {
        ...
        It "Get-MemoryDump should throw on non-existent file" {
            $badPath = if ($IsWindows) { 'C:\nonexistent\file.dmp' } else { '/nonexistent/file.dmp' }
            { Get-MemoryDump -Path $badPath -ErrorAction Stop } |
                Should -Throw
        }
